"""Compare MLP Hessian from specific layers on C4 Chinese.

Usage:
    python hessian_mlp_c4zh.py --device cuda:0 --layers 0 8
"""
...
CHUNK_ROWS = 4096
TOP_P = 0.9
MAX_TOKENS = 91154  # match wikitext 20-doc token count
...
def main():
    ...
    print(f"INT4 H-Optimal lm_head: MLP Hessian on C4 Chinese")
    print(f"Layers: {layers}  device={device}  BS={BLOCK_SIZE}  top_p={TOP_P}  max_tokens={MAX_TOKENS}\n", flush=True)
    ...
    for li in layers:
        ...
        print(f"  Evaluating on C4 Chinese...", flush=True)
        metrics = evaluate(model, tokenizer, ref_lm_head_weight=orig_lm_head_weight,
                           top_p=TOP_P, max_length=2048, max_tokens=MAX_TOKENS,
                           dataset_name="allenai/c4", dataset_config="zh",
                           split="validation")
