...
# Symmetric INT4 codebook: {-7, -6, ..., 0, ..., 6, 7}
Q_MAX = 7  # max quantized magnitude
D_0 = 0.5  # dead-zone boundary: |x|/s < 0.5 rounds to zero
VALID_BLOCK_SIZES = (16, 32)
...
def int4_quantize(x, s):
    """Symmetric INT4 quantization: ``round(clamp(x / s, -7, 7))``.
    ...
    Returns:
        Integer-valued tensor in ``[-7, 7]``, same shape as *x*.
    """
    return torch.clamp(torch.round(x / s), -Q_MAX, Q_MAX)


def int4_dequantize_block(quants, s):
    """Dequantize INT4 block values: ``dequant = quants * s``."""
    return quants * s


def compute_block_sse(x, s):
    """Per-block sum of squared INT4 quantization error."""
    if s.dim() == 1:
        s = s.unsqueeze(-1)
    quants = int4_quantize(x, s)
    dq = int4_dequantize_block(quants, s)
    return (x - dq).pow(2).sum(dim=-1)


def nvint4_naive(W, dim=-1, return_dequant=False):
    """Naive NVINT4 quantization: ``s = FP8_E4M3(max|x_i| / 7)`` per block.
    ...
    Returns:
        ...
        - **quants**: Integer values in ``[-7, 7]``. Same shape as *W*.
    ...
    quants = int4_quantize(x, scales.unsqueeze(-1))
    result = (
        scales.reshape(*batch_shape),
        quants.reshape(*batch_shape, block_size).movedim(-1, dim),
    )
    if return_dequant:
        dq = int4_dequantize_block(quants, scales.unsqueeze(-1))
        result = result + (dq.reshape(*batch_shape, block_size).movedim(-1, dim),)
    return result


def nvint4_optimal(W, dim=-1, return_dequant=False):
    """SSE-Optimal NVINT4 quantization via bounded search over FP8 E4M3 scales.

    Uses clipping and dead-zone bounds to reduce the search from 126
    FP8 candidates, with a fast-fail clipping check per candidate.
    ...
    quants = int4_quantize(x, best_s.unsqueeze(-1))
    result = (
        best_s.reshape(*batch_shape),
        quants.reshape(*batch_shape, block_size).movedim(-1, dim),
    )
    if return_dequant:
        dq = int4_dequantize_block(quants, best_s.unsqueeze(-1))
        result = result + (dq.reshape(*batch_shape, block_size).movedim(-1, dim),)
    return result


def _compute_block_hessian_error(x, s, H_blocks, M_dim, num_col_blocks, block_size):
    quants = int4_quantize(x, s.unsqueeze(-1))
    dq = int4_dequantize_block(quants, s.unsqueeze(-1))
    r = x - dq
    r_3d = r.reshape(M_dim, num_col_blocks, block_size)
    Hr = torch.einsum("jab,mjb->mja", H_blocks, r_3d)
    E_H = (r_3d * Hr).sum(dim=-1)
    return E_H.reshape(-1)


def nvint4_optimal_hessian(W, dim=-1, return_dequant=False, X=None, H_blocks=None):
    """Hessian-aware optimal NVINT4 scale search over FP8 E4M3 candidates.

    Selects the scale minimizing the Hessian-weighted error
    ``(x - sq)^T H (x - sq)`` per block, using SSE bounds for pruning.
    """
    ...
    if active.any():
        ...
        for s_val in all_scales:
            ...
            s_t = torch.tensor(s_f, device=x.device)
            quants_s = int4_quantize(x_active, s_t)
            dq_s = int4_dequantize_block(quants_s, s_t)
            r = x_active - dq_s
...
def nvint4_dequantize(scales, quants, dim=-1):
    """Dequantize NVINT4: ``dequant = quants * scales``."""
